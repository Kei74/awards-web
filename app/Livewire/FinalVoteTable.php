...
class FinalVoteTable extends Component implements HasTable, HasActions, HasSchemas
{
    ...
    public function table(Table $table): Table
    {
        ...
        $query = FinalVote::selectRaw('final_votes.entry_id, entries.name as entry_name, parents.name as parent_name, COUNT(*) as vote_count')
            ->join('entries', 'final_votes.entry_id', '=', 'entries.id')
            ->leftJoin('entries as parents', 'entries.parent_id', '=', 'parents.id')
            ->where('final_votes.category_id', $this->categoryId)
    ...
    {
        return view('livewire.final-vote-table');
    }
}
